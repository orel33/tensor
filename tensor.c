...
static uint
shape_size (uint ndim, uint *shape)
{
  assert (shape);
  uint size = 1;
...
static uint
calculate_index (uint ndim, uint *shape, uint *indices)
{
  assert (shape);
  assert (indices);
  uint index = 0;
...
tensor *
tensor_new (uint ndim, uint *shape, int *data)
{
  assert (shape);
...
tensor *
tensor_zeros (uint ndim, uint *shape)
{
...
tensor *
tensor_ones (uint ndim, uint *shape)
{
  tensor *t = tensor_new (ndim, shape, NULL);
  uint size = shape_size (ndim, shape);
  for (uint i = 0; i < size; i++)
...
int
tensor_get_element (tensor *t, uint *indices)
{
  assert (t);
  assert (indices);
...
void
tensor_set_element (tensor *t, uint *indices, int value)
{
  assert (t);
  assert (indices);
  uint index = calculate_index (t->ndim, t->shape, indices);
...
bool
tensor_equal (tensor *t1, tensor *t2)
{
  assert (t1);
  assert (t2);
  if (t1->ndim != t2->ndim)
...
tensor *
tensor_add (tensor *t1, tensor *t2)
{
  assert (t1);
  assert (t2);
...
tensor *
tensor_mul (tensor *t1, tensor *t2)
{
  assert (t1);
  assert (t2);
  assert (t1->ndim == t2->ndim);
...
tensor *
tensor_reshape (tensor *t, uint ndim, uint *shape)
{
  assert (t);
  assert (shape);
  uint size = shape_size (ndim, shape);
